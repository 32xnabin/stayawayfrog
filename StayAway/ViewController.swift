...
class ViewController: UIViewController {
    ...
    var smashSound:NSURL=NSURL()
    var backgroundSound:NSURL=NSURL()
    var backgroundSound1:NSURL=NSURL()
    ...
    internal func playBackgroundMusic(){
        ...
        backgroundSound = NSURL(fileURLWithPath: Bundle.main.path(forResource: "backmus", ofType: "mp3")!)
        do {
            try audioPlayer = AVAudioPlayer(contentsOf: backgroundSound as URL)
            audioPlayer.volume=0.17
        } catch {
        ...
        backgroundSound1 = NSURL(fileURLWithPath: Bundle.main.path(forResource: "helimusic", ofType: "mp3")!)
        do {
            try audioPlayer1 = AVAudioPlayer(contentsOf: backgroundSound1 as URL)
            audioPlayer1.volume=0.25
        } catch {
    ...
    var scene:GameScene!
    internal func prepareMovingBackground(){
    ...
    @objc func generateFire(timer: Timer) {
        ...
        //   let image: UIImage = UIImage(named: "throneball")!
        let image: UIImage = UIImage.gif(name: "firet2")!
...
var bgImage1: UIImageView?

fileprivate extension ViewController {
    func setupPlayerView() {
        
        var image: UIImage = UIImage.gif(name: "armyhelismall")!
        //  var image: UIImage = UIImage(named: "planeimage")!
        bgImage1 = UIImageView(image: image)
        playerView.bounds.size = CGSize(width: radius * 2, height: radius * 2)
        
        playerView.addSubview(bgImage1!)
        
        view.addSubview(playerView)
    }
    
    func startEnemyTimer() {
        enemyTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(generateFire(timer:)), userInfo: nil, repeats: true)
    }
    ...
    func displayGameOverAlert() {
        smashSound = NSURL(fileURLWithPath: Bundle.main.path(forResource: "blast", ofType: "mp3")!)
        do {
            try audioPlayer = AVAudioPlayer(contentsOf: smashSound as URL)
        ...
        //        bgImage?.image=UIImage.gif(name: "blood")!
        bgImage?.image=UIImage.gif(name: "explosion1")!
        
        scene.setBlackBackground()
        
        playerView.isHidden=true
        
        let (title, message) = getGameOverTitleAndMessage()
        let alert = UIAlertController(title: "Crashed", message: message, preferredStyle: .alert)
